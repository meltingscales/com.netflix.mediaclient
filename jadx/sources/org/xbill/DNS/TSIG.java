...
public class TSIG {
    ...
    public TSIGRecord generate(Message message, byte[] bArr, int i, TSIGRecord tSIGRecord, boolean z) {
        ...
        Duration b2 = (intValue < 0 || intValue > 32767) ? FUDGE : Duration.b(intValue);
        ...
        if (z) {
            ...
            dNSOutput.writeU16(PrivateKeyType.INVALID);
            dNSOutput.writeU32(0L);
            this.alg.toWireCanonical(dNSOutput);
        }
        writeTsigTimersVariables(instant, b2, dNSOutput);
        if (z) {
        ...
        if (i == 18) {
            ...
            writeTsigTime(this.clock.b(), dNSOutput2);
            bArr3 = dNSOutput2.toByteArray();
        } else {
            bArr3 = null;
        }
        return new TSIGRecord(this.name, PrivateKeyType.INVALID, 0L, this.alg, instant, b2, bArr4, message.getHeader().getID(), i, bArr3);
    }
    ...
    public int verify(Message message, byte[] bArr, TSIGRecord tSIGRecord, boolean z) {
        ...
        if (signature.length > macLength) {
            ...
            Instant b = this.clock.b();
            if (Duration.a(b, tsig.getTimeSigned()).d().compareTo(tsig.getFudge()) > 0) {
                logger.debug("BADTIME failure, now {} +/- tsig {} > fudge {}", b, tsig.getTimeSigned(), tsig.getFudge());
                return 18;
            }
            message.tsigState = 1;
            return 0;
    ...
    private static void writeTsigTimersVariables(Instant instant, Duration duration, DNSOutput dNSOutput) {
        ...
        dNSOutput.writeU16((int) duration.b());
    }
