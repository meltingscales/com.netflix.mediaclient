...
public final class ListMarkerBlock extends MarkerBlockImpl {
    ...
    public ListMarkerBlock(MarkdownConstraints markdownConstraints, ProductionHolder.Marker marker, char c) {
    ...
    public MarkerBlock.ProcessingResult doProcessToken(LookaheadText.Position position, MarkdownConstraints markdownConstraints) {
        ...
        if (position.getOffsetInCurrentLine() == -1) {
            ...
            if (!MarkdownConstraintsKt.extendsList(MarkdownConstraintsKt.applyToNextLineAndAddModifiers(getConstraints(), firstNonWhitespaceLinePos), getConstraints())) {
                return MarkerBlock.ProcessingResult.Companion.getDEFAULT();
            }
    ...
    public IElementType getDefaultNodeType() {
        ...
        if (c == '-' || c == '*' || c == '+') {
            return MarkdownElementTypes.UNORDERED_LIST;
        }
        return MarkdownElementTypes.ORDERED_LIST;
    }
}
