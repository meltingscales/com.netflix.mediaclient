...
public final class MediaCodecInfo {
    ...
    public boolean isFormatSupported(Format format) {
        ...
        if (isSampleMimeTypeSupported(format) && isCodecProfileAndLevelSupported(format, true)) {
            if (this.isVideo) {
                ...
                if (!z) {
                    logNoSupport("legacyFrameSize, " + format.width + InteractiveAnimation.ANIMATION_TYPE.X + format.height);
                }
                return z;
            }
            if (Util.SDK_INT >= 21) {
    ...
    private boolean isCodecProfileAndLevelSupported(Format format, boolean z) {
        ...
        if ("video/dolby-vision".equals(format.sampleMimeType)) {
            if ("video/avc".equals(this.mimeType)) {
                ...
                i = "video/hevc".equals(this.mimeType) ? 2 : 2;
            }
            intValue = i;
            intValue2 = 0;
        }
    ...
    public boolean isVideoSizeAndRateSupportedV21(int i, int i2, double d) {
        ...
        if (Util.SDK_INT >= 29) {
            ...
            if (areResolutionAndFrameRateCovered == 1) {
                logNoSupport("sizeAndRate.cover, " + i + InteractiveAnimation.ANIMATION_TYPE.X + i2 + "@" + d);
                return false;
            }
        }
        if (!areSizeAndRateSupportedV21(videoCapabilities, i, i2, d)) {
            if (i >= i2 || !needsRotatedVerticalResolutionWorkaround(this.name) || !areSizeAndRateSupportedV21(videoCapabilities, i2, i, d)) {
                logNoSupport("sizeAndRate.support, " + i + InteractiveAnimation.ANIMATION_TYPE.X + i2 + "@" + d);
                return false;
            }
            logAssumedSupport("sizeAndRate.rotated, " + i + InteractiveAnimation.ANIMATION_TYPE.X + i2 + "@" + d);
        }
        return true;
    }
    ...
    private static MediaCodecInfo.CodecProfileLevel[] estimateLegacyVp9ProfileLevels(MediaCodecInfo.CodecCapabilities codecCapabilities) {
        ...
        int i = intValue >= 180000000 ? 1024 : intValue >= 120000000 ? 512 : intValue >= 60000000 ? JSONzip.end : intValue >= 30000000 ? 128 : intValue >= 18000000 ? 64 : intValue >= 12000000 ? 32 : intValue >= 7200000 ? 16 : intValue >= 3600000 ? 8 : intValue >= 1800000 ? 4 : intValue >= 800000 ? 2 : 1;
