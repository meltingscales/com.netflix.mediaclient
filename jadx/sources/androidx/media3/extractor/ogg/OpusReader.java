...
final class OpusReader extends StreamReader {
    ...
    protected boolean readHeaders(ParsableByteArray parsableByteArray, long j, StreamReader.SetupData setupData) {
        if (peekPacketStartsWith(parsableByteArray, OPUS_ID_HEADER_SIGNATURE)) {
            ...
            setupData.format = new Format.Builder().setSampleMimeType("audio/opus").setChannelCount(channelCount).setSampleRate(VoipConfiguration.MAX_SAMPLERATE_48K).setInitializationData(buildInitializationData).build();
            return true;
        }
