...
public class CamColor {
    ...
    static void fromColorInViewingConditions(int i, ViewingConditions viewingConditions, float[] fArr, float[] fArr2) {
        ...
        float sqrt2 = ((float) Math.sqrt(pow5 / 100.0d)) * ((float) Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d)) * ((float) Math.pow((((((((float) (Math.cos((((((double) atan2) < 20.14d ? 360.0f + atan2 : atan2) * 3.141592653589793d) / 180.0d) + 2.0d) + 3.8d)) * 0.25f) * 3846.1538f) * viewingConditions.getNc()) * viewingConditions.getNcb()) * ((float) Math.sqrt((f16 * f16) + (f17 * f17)))) / (f19 + 0.305f), 0.9d));
        float flRoot2 = viewingConditions.getFlRoot() * sqrt2;
        float sqrt3 = (float) Math.sqrt((pow * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0f));
    ...
    int viewed(ViewingConditions viewingConditions) {
        ...
        float signum = ((Math.signum(f6) * (100.0f / viewingConditions.getFl())) * ((float) Math.pow(max, 2.380952380952381d))) / viewingConditions.getRgbD()[0];
        float signum2 = ((Math.signum(f7) * (100.0f / viewingConditions.getFl())) * pow2) / viewingConditions.getRgbD()[1];
        float signum3 = ((Math.signum(((f5 - (f3 * 220.0f)) - (f4 * 6300.0f)) / 1403.0f) * (100.0f / viewingConditions.getFl())) * ((float) Math.pow((float) Math.max(0.0d, (Math.abs(f) * 27.13d) / (400.0d - Math.abs(f))), 2.380952380952381d))) / viewingConditions.getRgbD()[2];
        ...
        float f13 = fArr3[2];
        float[] fArr4 = fArr[2];
        return ColorUtils.XYZToColor((f8 * signum) + (f9 * signum2) + (f10 * signum3), (f11 * signum) + (f12 * signum2) + (f13 * signum3), (signum * fArr4[0]) + (signum2 * fArr4[1]) + (signum3 * fArr4[2]));
    }
